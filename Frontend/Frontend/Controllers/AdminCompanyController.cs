...
public class AdminCompanyController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        var response = await client.GetAsync("http://localhost:5214/api/Company");
    ...
    public async Task<IActionResult> CreateCompany(CreateCompanyDto createBannerDto)
    {
        ...
        var response = await client.PostAsync("http://localhost:5214/api/Company", stringContent);
        if (response.IsSuccessStatusCode)
        {
    ...
    public async Task<IActionResult> UpdateCompany(string id)
    {
        var client = _httpClientFactory.CreateClient();
         ...
         var responseMesage = await client.GetAsync($"http://localhost:5214/api/Company/{id}");
    ...
    public async Task<IActionResult> UpdateCompany(UpdateCompanyDto updateBannerDto)
    {
        ...
        StringContent stringContent = new StringContent(jsonData, encoding: Encoding.UTF8, "application/json");
        var response = await client.PutAsync("http://localhost:5214/api/Company", stringContent);
    ...
    {
        var client = _httpClientFactory.CreateClient();
        var response = await client.DeleteAsync($"http://localhost:5214/api/Company?id={id}");
        if (response.IsSuccessStatusCode)
        {
