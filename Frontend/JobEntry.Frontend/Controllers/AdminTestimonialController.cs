...
public class AdminTestimonialController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        var response = await client.GetAsync("http://localhost:5202/api/Testimonial");
    ...
    public async Task<IActionResult> CreateTestimonial(CreateTestimonialDto createBannerDto)
    {
        ...
        var response = await client.PostAsync("http://localhost:5202/api/Testimonial", stringContent);
        if (response.IsSuccessStatusCode)
        {
    ...
    public async Task<IActionResult> UpdateTestimonial(string id)
    {
        var client = _httpClientFactory.CreateClient();
         ...
         var responseMesage = await client.GetAsync($"http://localhost:5202/api/Testimonial/{id}");
    ...
    public async Task<IActionResult> UpdateTestimonial(UpdateTestimonialDto updateBannerDto)
    {
        ...
        StringContent stringContent = new StringContent(jsonData, encoding: Encoding.UTF8, "application/json");
        var response = await client.PutAsync("http://localhost:5202/api/Testimonial", stringContent);
    ...
    {
        var client = _httpClientFactory.CreateClient();
        var response = await client.DeleteAsync($"http://localhost:5202/api/Testimonial?id={id}");
        if (response.IsSuccessStatusCode)
        {
