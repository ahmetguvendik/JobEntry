...
public class AdminBannerController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        var response = await client.GetAsync("http://localhost:5202/api/Banner");
    ...
    public async Task<IActionResult> CreateBanner(CreateBannerDto createBannerDto)
    {
        ...
        var response = await client.PostAsync("http://localhost:5202/api/Banner", stringContent);
        if (response.IsSuccessStatusCode)
        {
    ...
    public async Task<IActionResult> UpdateBanner(string id)
    {
        var client = _httpClientFactory.CreateClient();
         ...
         var responseMesage = await client.GetAsync($"http://localhost:5202/api/Banner/{id}");
    ...
    public async Task<IActionResult> UpdateBanner(UpdateBannerDto updateBannerDto)
    {
        ...
        StringContent stringContent = new StringContent(jsonData, encoding: Encoding.UTF8, "application/json");
        var response = await client.PutAsync("http://localhost:5202/api/Banner", stringContent);
    ...
    {
        var client = _httpClientFactory.CreateClient();
        var response = await client.DeleteAsync($"http://localhost:5202/api/Banner?id={id}");
        if (response.IsSuccessStatusCode)
        {
