...
public class AdminCategoryController : Controller
{
    ...
    public async Task<IActionResult> CreateCategory(CreateCategoryDto createBannerDto)
    {
        ...
        var response = await client.PostAsync("http://localhost:5202/api/Category", stringContent);
        if (response.IsSuccessStatusCode)
        {
    ...
    public async Task<IActionResult> UpdateCategory(string id)
    {
        var client = _httpClientFactory.CreateClient();
         ...
         var responseMesage = await client.GetAsync($"http://localhost:5202/api/Category/{id}");
    ...
    public async Task<IActionResult> UpdateCategory(UpdateCategoryDto updateBannerDto)
    {
        ...
        StringContent stringContent = new StringContent(jsonData, encoding: Encoding.UTF8, "application/json");
        var response = await client.PutAsync("http://localhost:5202/api/Category", stringContent);
    ...
    {
        var client = _httpClientFactory.CreateClient();
        var response = await client.DeleteAsync($"http://localhost:5202/api/Category?id={id}");
        if (response.IsSuccessStatusCode)
        {
